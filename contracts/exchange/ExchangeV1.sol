...
contract ExchangeV1 is Ownable, ExchangeDomainV1 {
  ...
  function validateBuyerFeeSig(
    ...
    require(
      prepareBuyerFeeMessage(order, buyerFee).recover(sig.v, sig.r, sig.s) == buyerFeeSigner,
  ...
  function transferWithFees(
    ...
    uint256 restValue = transferFeeToBeneficiary(firstType, from, value, sellerFee, buyerFee);
    if (
  ...
  function transferFeeToBeneficiary(
    ...
    (uint256 restValue, uint256 sellerFeeValue) = subFeeInBp(total, total, sellerFee);
  ...
  function getFeeSide(AssetType sellType, AssetType buyType) internal pure returns (FeeSide) {
    if (
      (sellType == AssetType.ERC721 || sellType == AssetType.ERC721Deprecated) &&
